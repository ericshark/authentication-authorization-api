...
def test_register_success(client):
    response = client.post("/auth/createUser", json={
        "username": "john",
        "name":"bob",
        "password": "secret123",
...
def test_update_password_success(auth_client):
    response = auth_client.post("/auth/updatePassword", json={
        "username": "john",
    ...
    login = auth_client.post("/auth/login", data={
        "username": "john",
        "password": "newpass456"
    })
...
def test_update_password_not_authenticated(client):
    response = client.post("/auth/updatePassword", json={
        "username":"john",
