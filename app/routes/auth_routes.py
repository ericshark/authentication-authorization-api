...
from app.auth.jwt_utils import create_jwt, refresh_hash, set_jwt_cookie
...
def register(db: db_dep, new_user: UserCreate, response: Response):
    try:
        logger.debug("Register point hit")
...
def logout_all(
    ...
    return get_auth_backend().logout_all(response, request, db, user)
...
def update_password(
    ...
    db.commit()
    get_auth_backend().logout_all(response, request, db, user)
